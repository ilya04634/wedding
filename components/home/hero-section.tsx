...
export function HeroSection({ guestName, settings }: HeroSectionProps) {
  ...
  return (
    <section className="relative isolate min-h-[calc(100svh-57px)] overflow-hidden px-4 py-10 text-center sm:min-h-[calc(100svh-65px)] sm:px-8 sm:py-20 lg:py-24">
      ...
      <div className="absolute -right-28 top-24 h-72 w-72 rounded-full bg-[radial-gradient(circle,_rgba(189,126,139,0.18),_rgba(189,126,139,0)_70%)] sm:-right-20 sm:top-16 sm:h-[32rem] sm:w-[32rem]" />
      ...
      <div className="garden-fade relative z-10 mx-auto flex max-w-4xl flex-col items-center">
        ...
        <div className="mt-7 max-w-full sm:mt-8">
          <p className="font-display text-4xl font-semibold uppercase tracking-[0.22em] text-[#4f5609] sm:text-6xl sm:tracking-[0.28em]">
            {settings.heroFamilyName}
          </p>
          <h1 className="font-script mt-1 text-balance text-[4.4rem] leading-[0.86] text-[#6c7411] sm:text-8xl lg:text-9xl">
            {settings.coupleNames}
          </h1>
        </div>
        <p className="font-display mx-auto mt-5 max-w-2xl text-balance text-2xl leading-tight text-[#4f5609] sm:text-4xl lg:text-5xl">
        ...
        <div className="mt-8 grid w-full max-w-2xl gap-3 text-sm text-[#4f5609] sm:mt-9 sm:grid-cols-2">
        ...
        <a
          href="#rsvp"
          className="mt-9 inline-flex min-h-12 w-full max-w-sm items-center justify-center rounded-full bg-[#4f5609] px-6 py-4 text-sm font-bold text-[#fbf3d9] shadow-[0_16px_35px_rgba(79,86,9,0.2)] transition hover:-translate-y-0.5 hover:bg-[#6c7411] sm:mt-10 sm:w-auto sm:px-8"
        >
