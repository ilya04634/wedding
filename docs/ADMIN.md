...
- Индивидуальный текст для каждого приглашения через колонку `invite_text`.
- Переключатель тона `informal_tone`: пусто/false = обращение на `вы`,
  `TRUE` = обращение на `ты`.
